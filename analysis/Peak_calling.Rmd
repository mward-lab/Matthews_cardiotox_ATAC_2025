...
# left_ventricle_Frip <-
#   left_ventricle_frip %>% 
#   dplyr::rename("sample"=X1,"lv_counts"= X2) %>% 
...
mutate(FRIP_fetal=c_muscle_counts/dedup_reads*100)
...
FRiP_first_run %>% 
  ...
  facet_wrap(~indv)+
  theme_classic()+
  ggtitle("FRiP Score across each sample")+
   scale_fill_manual(values=drug_pal_fac)
...
Frip_5_reads %>% 
  ...
  facet_wrap(~indv)+
  theme_classic()+
  ggtitle("FRiP Score across each sample for individual 5")+
   scale_fill_manual(values=drug_pal_fac)
