...
full_ATAC_FC <- toplist_full_ATAC$DOX_24 %>%
  left_join(., (exp_neargene_table %>%
                  dplyr::select(peakid,ENTREZID,ensembl_id,SYMBOL,dist_to_NG)), by=c("peak"="peakid") ) %>%
  mutate(mrc= if_else(ENTREZID %in% EAR_RNA$ENTREZID, "EAR", 
                      if_else(ENTREZID %in% ESR_RNA$ENTREZID, "ESR", 
                              if_else(ENTREZID %in% LR_RNA$ENTREZID, "LR",
                                      if_else(ENTREZID %in% NR_RNA$ENTREZID, "NR","not-mrc"))))) %>% 
...
Peak_MRC_table <- exp_neargene_table %>%
    distinct(peakid, .keep_all = TRUE) %>% 
    ...
    dplyr::select(chr:peakid,ENTREZID:mrc) 
  
  
Peak_MRC_table %>%    
    left_join(., (full_ATAC_FC_DOX24 %>% 
                  dplyr::select(peak,logFC,ENTREZID,mrc)), by=c("peakid"="peak", "ENTREZID"="ENTREZID"), suffix=c(".ATAC",".RNA")) %>% 
