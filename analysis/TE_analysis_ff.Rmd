...
Eight_group_TE %>% distinct(Peakid,.keep_all = TRUE) %>% 
  group_by(mrc, TEstatus) %>% 
  tally
# MRC_list(list=Eight_group_TE=Eight_group_TE)
...
TE_EAR_open_count <-
  Eight_group_TE %>%
  dplyr::filter(TEstatus =="TE_peak"& mrc=="EAR_open") %>%
  distinct() %>%
  count
TE_EAR_close_count <-
  Eight_group_TE %>%
  dplyr::filter(TEstatus =="TE_peak"& mrc=="EAR_close") %>%
  distinct() %>%
  count
Eight_group_TE %>%
   mutate(repClass=factor(repClass)) %>%
...
TE_ESR_open_count <-
  Eight_group_TE %>%
  dplyr::filter(TEstatus =="TE_peak"& mrc=="ESR_open") %>%
  distinct() %>%
  count
TE_ESR_close_count <-
  Eight_group_TE %>%
  dplyr::filter(TEstatus =="TE_peak"& mrc=="ESR_close") %>%
  distinct() %>%
  count
TE_ESR_OC_count <-
  Eight_group_TE %>%
  dplyr::filter(TEstatus =="TE_peak"& mrc=="ESR_OC") %>%
  distinct() %>%
  count
...
TE_LR_open_count <-
  Eight_group_TE %>%
  dplyr::filter(TEstatus =="TE_peak"& mrc=="LR_open") %>%
  distinct() %>%
  count
TE_LR_close_count <-
  Eight_group_TE %>%
  dplyr::filter(TEstatus =="TE_peak"& mrc=="LR_close") %>%
  distinct() %>%
  count
Eight_group_TE %>%
   mutate(repClass=factor(repClass)) %>%
...
##### VERY OLD... I learned a better way!
...
test2_5k <- join_overlap_intersect(peaks, gwas_snp_5k_gr) 
  # saveRDS(SNP_df,"data/Final_four_data/SNP_table_collapesed.RDS")
test2 %>% 
                              ...
                              if_else(Peakid %in% ESR_open$Peakid,"ESR_open",
                                      if_else(Peakid %in% ESR_close$Peakid,"ESR_close",
                                              if_else(Peakid %in% ESR_opcl$Peakid,"ESR_opcl",
                                                      if_else(Peakid %in% LR_open$Peakid,"LR_open",
                                                                      if_else(Peakid %in% LR_close$Peakid,"LR_close",
                                                                              if_else(Peakid %in% NR_df$Peakid,"NR",if_else(Peakid %in% ESR_clop$Peakid,"ESR_clop","not_mrc")))))))))) %>%
  # distinct(Peakid, .keep_all = TRUE) %>% 
  group_by(gwas,mrc) %>% 
  tally
                              ...
                              if_else(Peakid %in% ESR_open$Peakid,"ESR_open",
                                      if_else(Peakid %in% ESR_close$Peakid,"ESR_close",
                                              if_else(Peakid %in% ESR_C$Peakid,"ESR_opcl",
                                                      if_else(Peakid %in% LR_open$Peakid,"LR_open",
                                                                      if_else(Peakid %in% LR_close$Peakid,"LR_close",
                                                                              if_else(Peakid %in% NR_df$Peakid,"NR",if_else(Peakid %in% ESR_D$Peakid,"ESR_clop","not_mrc")))))))))) %>%
...
SNPanno %>% 
  ...
  kable_styling(full_width = FALSE, font_size = 14) %>% 
  scroll_box(height = "500px")
# final_SNP_df <- SNP_df %>% left_join(.,Collapsed_new_peaks, by = c("Peakid"="Peakid"))
```
...
SNP_status_list <- Eight_group_TE %>% 
...
SNP_status_list_5k <- Eight_group_TE %>% 
  mutate(mrc=if_else(Peakid %in% ESR_C$Peakid,"ESR_opcl",                 if_else(Peakid %in% ESR_D$Peakid,"ESR_clop",mrc))) %>% 
  distinct(Peakid,mrc) %>% 
...
```{r SNP chissquare}
matrix_list_SNP <- list("SNP"=SNP_mat, "ARR"=ARR_mat,"CAD"= CAD_mat,"HD"= HD_mat,"HF"=HF_mat,"MI"=MI_mat,"DCM"=DCM_mat,"base eQTL_mat"=base_eQTL_mat ,"DOXres eQTL"=eQTL_mat,"SNP_5k"=SNP_mat_5k, "ARR_5k"=ARR_mat_5k,"CAD_5k"= CAD_mat_5k,"HD_5k"= HD_mat_5k,"HF_5k"=HF_mat_5k,"MI_5k"=MI_mat_5k,"DCM_5k"=DCM_mat_5k, "base eQTL_mat_5k"=base_eQTL_mat_5k ,"DOXres eQTL_5k"=eQTL_mat_5k)
...
names_list <- names(matrix_list_SNP)
# Loop through each matrix in the list
for (matrix_name in names_list) {
  current_matrix <- matrix_list_SNP[[matrix_name]]
...
# saveRDS(results_SNP,"data/Final_four_data/SNP_chisquare_results.RDS")
print(results_SNP) %>% 
...
# Loop through each matrix in the list
for (matrix_name in names(matrix_list_SNP)) {
  current_matrix <- matrix_list_SNP[[matrix_name]]
  n_rows <- nrow(current_matrix)
...
# saveRDS(results_SNP_or,"data/Final_four_data/SNP_oddsratio_results.RDS")
...
# results_or <- readRDS("data/Final_four_data/OR_results_TE_df.RDS")
# Print the resulting dataframe
print(results_or) %>% 
...
col_fun_OR = colorRamp2(c(0,.8,1,1.5,3,4), c("#BC9BFF","#BC9BFF","white","lightgreen","green3","green3" ))
...
```{r ARRHF SNP, eval=FALSE, include=FALSE}
gwas_snp_20k_gr <- gwas_snp_list %>% 
   mutate(CHR_ID=as.numeric(CHR_ID), CHR_POS=as.numeric(CHR_POS)) %>% 
  na.omit() %>%
   mutate(start=CHR_POS-10000, end=CHR_POS+10000, chr=paste0("chr",CHR_ID)) %>% 
  GRanges()
gwas_snp_50k_gr <- gwas_snp_list %>% 
   mutate(CHR_ID=as.numeric(CHR_ID), CHR_POS=as.numeric(CHR_POS)) %>% 
  na.omit() %>%
   mutate(start=CHR_POS-25000, end=CHR_POS+25000, chr=paste0("chr",CHR_ID)) %>% 
  GRanges()

ARR_HF_gr <- join_overlap_intersect(peaks, gwas_snp_gr) 

ARR_HF_gr_10k <- join_overlap_intersect(peaks, gwas_snp_5k_gr) 
ARR_HF_gr_20k <- join_overlap_intersect(peaks, gwas_snp_20k_gr) 
ARR_HF_gr_50k <- join_overlap_intersect(peaks, gwas_snp_50k_gr) 


# ARR_HF_snp_list <- 
  direct <- SNP_df %>% 
    dplyr::filter(grepl("ARR",gwas)|grepl("HF",gwas)) %>% 
  dplyr::select(Peakid,SNPS,gwas,mrc) 
...
ARR_HF_gr_50k %>% 
    as.data.frame() %>% 
    dplyr::filter(gwas=="ARR"|gwas=="HF") %>% 
    left_join(.,(Collapsed_new_peaks %>% dplyr::select(Peakid,NCBI_gene,SYMBOL)), by=c("Peakid"="Peakid"))
...
Peak_TE_overlapbreakdown %>%  mutate(mrc=if_else(
  Peakid %in% EAR_open$Peakid, "EAR_open",if_else(
    Peakid %in% EAR_close$Peakid, "EAR_close", if_else(
      Peakid %in% ESR_open$Peakid,"ESR_open", if_else(
        Peakid %in% ESR_close$Peakid,"ESR_close",
    if_else(
      Peakid %in% ESR_opcl$Peakid,"ESR_opcl",        if_else(
        Peakid %in% LR_open$Peakid,"LR_open",
           if_else(
             Peakid %in% LR_close$Peakid,"LR_close",if_else(
               Peakid %in% NR_df$Peakid,"NR",if_else(
                 Peakid %in% ESR_clop$Peakid,"ESR_clop","not_mrc")))))))))) %>%
  mutate(mrc=factor(mrc, levels = c("EAR_open",
                    "ESR_open", "LR_open", "ESR_opcl", "EAR_close","ESR_close", "LR_close","ESR_clop","NR","not_mrc"))) %>% 
  dplyr::filter(repClass=="Retroposon") %>% 
  dplyr::filter(dist_to_NG>-20000&dist_to_NG<20000) %>% 
  ggplot(., aes (x=dist_to_NG,y=mrc, fill=mrc))+
  geom_density_ridges()+
  theme_classic()+
  # xlim(-50000,50000)+
  ggtitle("Density of SVA element containing peaks from TSS 20kb filter")+
  xlab("Distancs from TSS (bp)")+
  facet_wrap(~repName)
