...
**note** these numbers include peaks that are not classified by an MRC.
 ...
 Tables 3 above used the "new" way of sub-setting (keeping all metadata organized vs the sub-setting code I found on the internet).  Just to verify the numbers were the same, I ran the data using the old method and compared the numbers.  They are identical (Table 6) to each other.  Success!
...
TE_mrc_status_list <- Peak_TE_overlapbreakdown %>% 
 ...
 Peak_TE_overlapbreakdown %>% 
   ...
   rbind((TE_mrc_status_list %>%
            ### adding a line to only show single peaks
            distinct(Peakid,mrc,TEstatus, .keep_all = TRUE))) %>% 
   mutate(repClass=factor(repClass)) %>%
...
Peak_TE_overlapbreakdown %>% 
  ...
  mutate(TEstatus=if_else(is.na(per_ol),"not_TE_peak",if_else(per_ol>per_cov, "TE_peak",
                          if_else(per_ol<per_cov,"not_TE_peak","not_TE_peak")))) %>% 
mutate(TEstatus=factor(TEstatus, levels = c("TE_peak","not_TE_peak")))%>% 
   ggplot(., aes(x=mrc, fill= TEstatus))+
  geom_bar(position="fill", col="black")+
  theme_classic()+
...
TE_mrc_status_list %>% 
  distinct(Peakid, .keep_all = TRUE) %>% 
   ...
   dplyr::filter(mrc != "not_mrc") %>% 
  mutate(mrc=factor(mrc, levels = c("EAR","ESR", "LR", "NR","all_peaks"))) %>%
    mutate(TEstatus=if_else(is.na(per_ol),"not_TE_peak",
                            if_else(per_ol>per_cov, "TE_peak", if_else(per_ol<per_cov, "not_TE_peak","not_TE_peak")))) %>% 
mutate(TEstatus=factor(TEstatus, levels = c("TE_peak","not_TE_peak")))%>% 
   dplyr::filter(mrc != "not_mrc") %>% 
  mutate(mrc=factor(mrc, levels = c("EAR","ESR", "LR", "NR","all_peaks"))) %>% 
     group_by(mrc, TEstatus) %>% 
