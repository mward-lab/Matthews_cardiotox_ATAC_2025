...
```{r SEA data, eval=FALSE, include=FALSE}

ggplot(hits_ESR_250, aes(start))+
  # geom_histogram(bins = 100)
  geom_density()

ggplot(hits_NR_ZNF384, aes(relScore))+
  geom_histogram(bins = 100)
...
hits_LR_PRDM1 %>%
  ...
  rbind(.,(hits_NR_PRDM1  %>%
  mutate(id_mrc="NR"))) %>%
   rbind(.,(hits_ESR_PRDM1 %>%
  mutate(id_mrc="ESR"))) %>%
   rbind(.,(hits_EAR_PRDM1  %>%
  mutate(id_mrc="EAR"))) %>%
  dplyr::filter(relScore > .60)  %>% 
  mutate(position=start-1000, line ="cut") %>% 
  # rbind((hits_LR_PRDM1 %>% mutate(position=start-1000, line="full", id_mrc="LR") )) %>% 
  #  rbind((hits_EAR_PRDM1 %>% mutate(position=start-1000, line="full",id_mrc="EAR") )) %>% 
  #  rbind((hits_ESR_PRDM1 %>% mutate(position=start-1000, line="full",id_mrc="ESR") )) %>% 
  #  rbind((hits_NR_PRDM1 %>% mutate(position=start-1000, line="full",id_mrc="NR") )) %>% 
  
  ggplot(.,aes(position, color=id_mrc, linetype = line))+
  geom_density(size=1) +
...
ESR_sea %>% 
  ...
  geom_point(aes(x=`TP%`))+
 theme_bw()+
  ggtitle("expressed ESR enriched motifs")
