...
# sea_LRU <- sea_LRU%>%
#   slice_head(n = length(.$ID)-3)
# saveRDS(sea_LRU, "data/xstreme/sea_LRU.RDS")
# sea_LR <- sea_LR%>%
#   slice_head(n = length(.$ID)-3)
# saveRDS(sea_LR, "data/xstreme/sea_LR.RDS")

# saveRDS(xstreme_EARD, "data/xstreme/xstreme_EARD.RDS")
# saveRDS(xstreme_EARU, "data/xstreme/xstreme_EARU.RDS")
# saveRDS(xstreme_ESRD, "data/xstreme/xstreme_ESRD.RDS")
# saveRDS(xstreme_ESRU, "data/xstreme/xstreme_ESRU.RDS")
# saveRDS(xstreme_LRD, "data/xstreme/xstreme_LRD.RDS")
# saveRDS(xstreme_LRU, "data/xstreme/xstreme_LRU.RDS")
...
sea_EARD <- readRDS("data/xstreme/sea_EARD.RDS")
sea_ESRD <- readRDS("data/xstreme/sea_ESRD.RDS")
sea_LRD <- readRDS("data/xstreme/sea_LRD.RDS")
sea_EARU <- readRDS("data/xstreme/sea_EARU.RDS")
sea_ESRU <- readRDS("data/xstreme/sea_ESRU.RDS")
sea_LRU <- readRDS("data/xstreme/sea_LRU.RDS")




xstreme_EARD <- readRDS("data/xstreme/xstreme_EARD.RDS")
xstreme_EARU <- readRDS("data/xstreme/xstreme_EARU.RDS")
xstreme_ESRD <- readRDS("data/xstreme/xstreme_ESRD.RDS")
xstreme_ESRU <- readRDS("data/xstreme/xstreme_ESRU.RDS")
xstreme_LRD <- readRDS("data/xstreme/xstreme_LRD.RDS")
xstreme_LRU <- readRDS("data/xstreme/xstreme_LRU.RDS")
...
### Enrichment Hyper peaks to NR hyper peaks
```{r simmotifUP, message=FALSE, warning=FALSE}
xstreme_EARU %>% 
  dplyr::select(SIM_MOTIF,ALT_ID, ID,SEA_PVALUE,EVALUE) %>% 
  arrange(.,EVALUE) %>% 
  # separate(SIM_MOTIF, into= c("SIM_MOTIF", "NAME"), sep= " ") %>% 
  # dplyr::filter(., EVALUE<0.05) %>% 
  dplyr::mutate_if(is.numeric, funs(as.character(signif(., 3)))) %>%
  kable(., caption = "Enriched motifs in EAR v NR hyper") %>%
  kable_paper("striped", full_width = TRUE) %>%
  kable_styling(full_width = FALSE, font_size = 16) %>%
  scroll_box(height = "500px")
...
xstreme_ESRU %>% 
 dplyr::select(SIM_MOTIF,ALT_ID, ID,SEA_PVALUE,EVALUE) %>% 
  ...
  # dplyr::filter(., EVALUE<0.05) %>% 
  dplyr::mutate_if(is.numeric, funs(as.character(signif(., 3)))) %>%
  kable(., caption = "Enriched motifs in ESR v NR hyper") %>%
  kable_paper("striped", full_width = TRUE) %>%
  kable_styling(full_width = FALSE, font_size = 16) %>%
  scroll_box(height = "800px")
...
xstreme_LRU %>% 
  ...
  # separate(SIM_MOTIF, into= c("SIM_MOTIF", "NAME"), sep= " ") %>% 
  # dplyr::filter(., EVALUE<0.05) %>% 
  kable(., caption = "Enriched motifs  (EVALUE < 0.05) in LR v NR hyper") %>%
  kable_paper("striped", full_width = TRUE) %>%
  kable_styling(full_width = FALSE, font_size = 16) %>%
  scroll_box(height = "800px")
### selecting just the list of names and checking for expression
...
gene_list_SEA_hyper <- xstreme_EARU %>% 
 dplyr::select(SIM_MOTIF,ALT_ID, ID,SEA_PVALUE,EVALUE) %>% 
  arrange(.,EVALUE) %>% 
  separate(SIM_MOTIF, into= c("SIM_MOTIF", "NAME"), sep= " ") %>% 
  mutate(MCR="EAR") %>% 
  bind_rows(xstreme_ESRU %>% 
 dplyr::select(SIM_MOTIF,ALT_ID, ID,SEA_PVALUE,EVALUE) %>% 
  arrange(.,EVALUE) %>% 
  separate(SIM_MOTIF, into= c("SIM_MOTIF", "NAME"), sep= " ") %>% 
 mutate(MCR="ESR")) %>% 
  bind_rows(xstreme_LRU %>% 
  dplyr::select(SIM_MOTIF,ALT_ID, ID,SEA_PVALUE,EVALUE) %>% 
  arrange(.,EVALUE) %>% 
  separate(SIM_MOTIF, into= c("SIM_MOTIF", "NAME"), sep= " ") %>% 
    mutate(MCR="LR")) %>% 
...
### Enrichment Hypo peaks to NR hypo peaks
```{r simmotifDOWN, message=FALSE, warning=FALSE}
xstreme_EARD %>% 
  dplyr::select(SIM_MOTIF,ALT_ID, ID,SEA_PVALUE,EVALUE) %>% 
  arrange(.,EVALUE) %>% 
  # separate(SIM_MOTIF, into= c("SIM_MOTIF", "NAME"), sep= " ") %>% 
  # dplyr::filter(., EVALUE<0.05) %>% 
  dplyr::mutate_if(is.numeric, funs(as.character(signif(., 3)))) %>%
  kable(., caption = "Enriched motifs in EAR v NR hypo") %>%
  kable_paper("striped", full_width = TRUE) %>%
  kable_styling(full_width = FALSE, font_size = 16) %>%
  scroll_box(height = "500px")
...
xstreme_ESRD %>% 
 dplyr::select(SIM_MOTIF,ALT_ID, ID,SEA_PVALUE,EVALUE) %>% 
  ...
  # dplyr::filter(., EVALUE<0.05) %>% 
  dplyr::mutate_if(is.numeric, funs(as.character(signif(., 3)))) %>%
  kable(., caption = "Enriched motifs in ESR v NR hypo") %>%
  kable_paper("striped", full_width = TRUE) %>%
  kable_styling(full_width = FALSE, font_size = 16) %>%
  scroll_box(height = "800px")
...
xstreme_LRD %>% 
  ...
  # separate(SIM_MOTIF, into= c("SIM_MOTIF", "NAME"), sep= " ") %>% 
  # dplyr::filter(., EVALUE<0.05) %>% 
  kable(., caption = "Enriched motifs  (EVALUE < 0.05) in LR v NR hypo") %>%
  kable_paper("striped", full_width = TRUE) %>%
  kable_styling(full_width = FALSE, font_size = 16) %>%
  scroll_box(height = "800px")
### selecting just the list of names and checking for expression
...
gene_list_SEA_hypo <- xstreme_EARD %>% 
 dplyr::select(SIM_MOTIF,ALT_ID, ID,SEA_PVALUE,EVALUE) %>% 
  arrange(.,EVALUE) %>% 
  separate(SIM_MOTIF, into= c("SIM_MOTIF", "NAME"), sep= " ") %>% 
  mutate(MCR="EAR") %>% 
  bind_rows(xstreme_ESRD %>% 
 dplyr::select(SIM_MOTIF,ALT_ID, ID,SEA_PVALUE,EVALUE) %>% 
  arrange(.,EVALUE) %>% 
  separate(SIM_MOTIF, into= c("SIM_MOTIF", "NAME"), sep= " ") %>% 
 mutate(MCR="ESR")) %>% 
  bind_rows(xstreme_LRD %>% 
  dplyr::select(SIM_MOTIF,ALT_ID, ID,SEA_PVALUE,EVALUE) %>% 
  arrange(.,EVALUE) %>% 
  separate(SIM_MOTIF, into= c("SIM_MOTIF", "NAME"), sep= " ") %>% 
    mutate(MCR="LR")) %>% 
...
ggplot(., aes (y= reorder(motif_name,log10Evalue))) +
  ...
  scale_x_continuous(sec.axis = sec_axis(transform= ~.,name="Percent of peaks with motif"))+
  # geom_text(aes())
  theme_classic()+
...
```{r enriched motifs EAR hyper}

xstreme_EARU %>% 
  dplyr::select(SOURCE:CONSENSUS,CLUSTER,SITES,SEA_PVALUE,EVALUE,SIM_MOTIF) %>%
  mutate(CONSENSUS=gsub('[[:digit:]]+-', '', CONSENSUS)) %>% 
  left_join(., (sea_EARU %>% dplyr::select(ID:ENR_RATIO,EVALUE)), by= c("ALT_ID"="ALT_ID","CONSENSUS"="CONSENSUS", "ID"="ID")) %>% 
  dplyr::select(CLUSTER, SITES,SIM_MOTIF,ALT_ID, ID,EVALUE.x, TP:'FP%')%>% 
  separate(SIM_MOTIF, into= c("SIM_MOTIF", "NAME"), sep= " ") %>% 
  arrange(.,EVALUE.x) %>%
 # dplyr::mutate_if(is.numeric, funs(as.character(signif(., 3))))#%>%
...
ggplot(., aes (y= reorder(motif_name,log10Evalue))) +
  geom_col(aes(x=log10Evalue),fill="pink2") +
   geom_point(aes(x=`TP%`*5), size =5)+
   # geom_line(aes(x=`TP%`,y= motif_name, group=log10Evalue))+
  scale_x_continuous(name="-log(10) E Value",
                     sec.axis = sec_axis( ~./5,name="Percent of peaks with motif"))+
  # geom_text(aes())
  theme_classic()+
  ylab("Enriched TF motif")+
  ggtitle("Early-acute response peaks- hyper accessible")
 
```

##### EAR hypo

```{r enriched motifs EAR hypo}

xstreme_EARD %>% 
  dplyr::select(SOURCE:CONSENSUS,CLUSTER,SITES,SEA_PVALUE,EVALUE,SIM_MOTIF) %>%
  mutate(CONSENSUS=gsub('[[:digit:]]+-', '', CONSENSUS))%>% 
  left_join(., (sea_EARD %>% dplyr::select(ID:ENR_RATIO,EVALUE)), by= c("ALT_ID"="ALT_ID","CONSENSUS"="CONSENSUS", "ID"="ID")) %>% 
  dplyr::select(CLUSTER, SITES,SIM_MOTIF,ALT_ID, ID,EVALUE.x, TP:'FP%')%>% 
  separate(SIM_MOTIF, into= c("SIM_MOTIF", "NAME"), sep= " ") %>% 
  arrange(.,EVALUE.x) %>%
 # dplyr::mutate_if(is.numeric, funs(as.character(signif(., 3))))#%>%
...
ggplot(., aes (y= reorder(motif_name,log10Evalue))) +
   geom_col(aes(x=log10Evalue),fill="pink2") +
   geom_point(aes(x=`TP%`/1.25), size =5)+
   # geom_line(aes(x=`TP%`,y= motif_name, group=log10Evalue))+
  scale_x_continuous(name="-log(10) E value",
                     sec.axis = sec_axis( ~.*1.25,name="Percent of peaks with motif"))+
  theme_classic()+
  ylab("Enriched TF motif")+
  ggtitle("Early-acute response peaks hypo accessible")
...
##### ESR hyper
```{r enriched motifs ESR hyper, fig.height=8}
xstreme_ESRU %>% 
   dplyr::select(SOURCE:CONSENSUS,CLUSTER,SITES,SEA_PVALUE,EVALUE,SIM_MOTIF) %>%
  mutate(CONSENSUS=gsub('[[:digit:]]+-', '', CONSENSUS))%>% 
  left_join(., (sea_ESRU %>% dplyr::select(ID:ENR_RATIO,EVALUE)), by= c("ID"="ID","CONSENSUS"="CONSENSUS" )) %>% 
  dplyr::select(CLUSTER, SITES,SIM_MOTIF,ID,EVALUE.x, TP:'FP%')%>% 
  separate(SIM_MOTIF, into= c("SIM_MOTIF", "NAME"), sep= " ") %>% 
  arrange(.,EVALUE.x) %>%
  mutate(log10Evalue= log(EVALUE.x, base = 10)*(-1)) %>% 
   group_by(NAME) %>% 
  ...
  # ungroup() %>% 
  mutate(motif_name= gsub("[()]","",NAME), mrc="EAR") %>% 
  mutate(motif_name=if_else(is.na(motif_name),ID,
                            if_else(motif_name=="ZSCAN4",paste(motif_name, RowGroup, sep="_"),
                                    if_else(motif_name=="KLF9",paste(motif_name, RowGroup, sep="_"),motif_name))))%>%
 # dplyr::filter(!is.na(motif_name)) %>% 
  # group_by(mrc) %>% 
ggplot(., aes (y= reorder(motif_name,log10Evalue))) +
  geom_col(aes(x=log10Evalue),fill="lightgreen") +
  ...
  # geom_text(aes())
   scale_x_continuous(name="-log(10) E value",
                     sec.axis = sec_axis( ~./9,name="Percent of peaks with motif"))+
  theme_classic()+
  ylab("Enriched TF motif")+
  ggtitle("Early-sustained response peaks hyper accessible")
...
##### ESR hypo
```{r enriched motifs ESR hypo, fig.height=8}
xstreme_ESRD %>% 
   dplyr::select(SOURCE:CONSENSUS,CLUSTER,SITES,SEA_PVALUE,EVALUE,SIM_MOTIF) %>%
  mutate(CONSENSUS=gsub('[[:digit:]]+-', '', CONSENSUS))%>%
  left_join(., (sea_ESRD %>% dplyr::select(ID:ENR_RATIO,EVALUE)), by= c("ID"="ID","CONSENSUS"="CONSENSUS" )) %>% 
  dplyr::select(CLUSTER, SITES,SIM_MOTIF,ID,EVALUE.x, TP:'FP%')%>% 
  separate(SIM_MOTIF, into= c("SIM_MOTIF", "NAME"), sep= " ") %>% 
  arrange(.,EVALUE.x) %>%
 # dplyr::mutate_if(is.numeric, funs(as.character(signif(., 3))))#%>%
  ...
  mutate(motif_name= gsub("[()]","",NAME), mrc="EAR")# %>% 
  mutate(motif_name=if_else(is.na(motif_name),ID,motif_name)) %>% 
  mutate(motif_name=if_else(is.na(motif_name),ID,if_else(motif_name=="ZNF449",paste(motif_name, RowGroup, sep="_"),motif_name)))%>%
# dplyr::filter(!is.na(motif_name)) %>% 
  # group_by(mrc) %>% 
ggplot(., aes (y= reorder(motif_name,log10Evalue))) +
  geom_col(aes(x=log10Evalue),fill="lightgreen") +
   geom_point(aes(x=`TP%`), size =3)+
   # geom_line(aes(x=`TP%`,y= motif_name, group=log10Evalue))+
  # geom_text(aes())
   scale_x_continuous(name="-log(10) E value",
                     sec.axis = sec_axis( ~.,name="Percent of peaks with motif"))+
  theme_classic()+
  ylab("Enriched TF motif")+
  ggtitle("Early-sustained response peaks hypo accessible")
...
xstreme_LR_v_NR %>% 
    dplyr::select(SOURCE:CONSENSUS,CLUSTER,SITES,SEA_PVALUE,EVALUE,SIM_MOTIF) %>%
  mutate(CONSENSUS=gsub('[[:digit:]]+-', '', CONSENSUS))%>% 
  left_join(., (sea_LRD %>% dplyr::select(ID:ENR_RATIO,EVALUE)), by= c("ALT_ID"="ALT_ID","ID"="ID","CONSENSUS"="CONSENSUS" )) %>% 
  dplyr::select(CLUSTER, SITES,SIM_MOTIF,ID,EVALUE.x, TP:'FP%')%>% 
  separate(SIM_MOTIF, into= c("SIM_MOTIF", "NAME"), sep= " ") %>% 
  arrange(.,EVALUE.x) %>%
 # dplyr::mutate_if(is.numeric, funs(as.character(signif(., 3))))#%>%
...
```{r enriched motifs LR hyper}
xstreme_LRU %>% 
    dplyr::select(SOURCE:CONSENSUS,CLUSTER,SITES,SEA_PVALUE,EVALUE,SIM_MOTIF) %>%
  mutate(CONSENSUS=gsub('[[:digit:]]+-', '', CONSENSUS))%>% 
   left_join(., (sea_LRU %>% dplyr::select(ID:ENR_RATIO,EVALUE)), by= c("ALT_ID"="ALT_ID","ID"="ID","CONSENSUS"="CONSENSUS" )) %>% 
  dplyr::select(CLUSTER, SITES,SIM_MOTIF,ID,EVALUE.x, TP:'FP%')%>% 
  separate(SIM_MOTIF, into= c("SIM_MOTIF", "NAME"), sep= " ") %>% 
  arrange(.,EVALUE.x) %>%
 # dplyr::mutate_if(is.numeric, funs(as.character(signif(., 3))))#%>%
...
ggplot(., aes (y= reorder(motif_name,log10Evalue))) +
  geom_col(aes(x=log10Evalue),fill="cyan") +
    geom_point(aes(x=`TP%`/1.5), size =5)+
   # geom_line(aes(x=TP, group=mrc))+
  # geom_text(aes())
   scale_x_continuous(name="-log(10) E value",
                     sec.axis = sec_axis( ~.*1.5,name="Percent of peaks with motif"))+
  theme_classic()+
  ylab("Enriched TF motif")+
  ggtitle("Late response peaks hyper accessible")
...
ggplot(., aes (y= reorder(motif_name,log10Evalue))) +
  geom_col(aes(x=log10Evalue),fill="cyan") +
    geom_point(aes(x=`TP%`/1.5), size =5)+
  geom_point(aes(x=`FP%`/1.5), size =4, col="gray")+
   # geom_line(aes(x=`TP%`,y= motif_name, group=log10Evalue))+
  scale_x_continuous(name="-log(10) E Value",
                     sec.axis = sec_axis( ~.*1.5,name="Percent of peaks with motif"))+
  theme_classic()+
  ylab("Enriched TF motif")+
  ggtitle("Late response peaks hypo accessible")
...
```{r creating U and D}
### need to load Top list of all aTAC data.

toplist_full_ATAC <- readRDS("data/DEG_toplist_sep_n45.RDS")
# DOX_24 <- toplist_full_ATAC$DOX_24
median_24_lfc <- readRDS("data/median_24_lfc.RDS") %>% ungroup()
median_3_lfc <- readRDS("data/median_3_lfc.RDS") %>% ungroup()

# DOX_24 %>% dplyr::filter(logFC>0) %>% 
#     separate_wider_delim(.,peak, names = c( "chr","start","end"), delim= ".",cols_remove = FALSE) %>%
#      dplyr::rename("name"="peak") %>% 
#      mutate(strand=".") %>% 
#      GRanges() %>% 
#      rtracklayer::export(., con="data/hyper_files/tot_hyper.bed",format="bed", ignore.strand = FALSE)
# 
# DOX_24 %>% dplyr::filter(logFC<0) %>% 
#     separate_wider_delim(.,peak, names = c( "chr","start","end"), delim= ".",cols_remove = FALSE) %>%
#      dplyr::rename("name"="peak") %>% 
#      mutate(strand=".") %>% 
#      GRanges() %>% 
#      rtracklayer::export(., con="data/hypo_files/tot_hypo.bed",format="bed", ignore.strand = FALSE)

# as.GRanges(toplist_full_ATAC$DOX_24)
...
# tot_hypo <- DOX_24 %>% dplyr::filter(logFC<0)

EARU_3h <-  median_3_lfc %>%
  dplyr::filter(peak %in% EAR_df$id) %>% 
    dplyr::filter(median>0) 

EARU_3h_gr <- EARU_3h%>% 
  separate(.,col=peak, into=c("seqnames","start","stop"),  remove= FALSE) %>% GRanges()

EARU_24h <-  median_24_lfc %>%
  dplyr::filter(peak %in% EAR_df$id) %>% 
    dplyr::filter(median>0) 
EARU_24h_gr <- EARU_24h%>% 
  separate(.,col=peak, into=c("seqnames","start","stop"),  remove= FALSE) %>% GRanges()

EARD_3h <-  median_3_lfc %>%
  dplyr::filter(peak %in% EAR_df$id) %>% 
    dplyr::filter(median<0) 
EARD_3h_gr <- EARD_3h%>% 
  separate(.,col=peak, into=c("seqnames","start","stop"),  remove= FALSE) %>% GRanges()

EARD_24h <-  median_24_lfc %>%
  dplyr::filter(peak %in% EAR_df$id) %>% 
    dplyr::filter(median<0) 

EARD_24h_gr <- EARD_24h%>% 
  separate(.,col=peak, into=c("seqnames","start","stop"),  remove= FALSE) %>% GRanges()

ESRU_3h <-  median_3_lfc %>%
...
ESRU_24h <-  median_24_lfc %>%
  dplyr::filter(peak %in% ESR_df$id) %>% 
    dplyr::filter(median>0) 
ESRU_24h_gr <- ESRU_24h%>% 
  separate(.,col=peak, into=c("seqnames","start","stop"),  remove= FALSE) %>% GRanges()

ESRD_3h <-  median_3_lfc %>%
  dplyr::filter(peak %in% ESR_df$id) %>% 
    dplyr::filter(median<0) 
ESRD_3h_gr <- ESRD_3h%>% 
  separate(.,col=peak, into=c("seqnames","start","stop"),  remove= FALSE) %>% GRanges()

ESRD_24h <-  median_24_lfc %>%
  dplyr::filter(peak %in% ESR_df$id) %>% 
    dplyr::filter(median<0) 

ESRD_24h_gr <- ESRD_24h%>% 
  separate(.,col=peak, into=c("seqnames","start","stop"),  remove= FALSE) %>% GRanges()
...
```{r saving files, eval=FALSE, include=TRUE}
# script to convert into each file needed for up and down
df_names <- list(EAR_df,ESR_df,LR_df,NR_df)
names(df_names) <- c("EAR_df","ESR_df","LR_df","NR_df")

for (i in 1:4){
a<- df_names[[i]]

median_24_lfc  %>%
  dplyr::filter(peak %in% a$id) %>% 
  dplyr::filter(median>0) %>% 
  separate_wider_delim(.,peak, names = c( "chr","start","end"), delim= ".",cols_remove = FALSE) %>%
     dplyr::rename("name"="peak") %>% 
     mutate(strand=".") %>% 
     GRanges() %>%
     rtracklayer::export(., con=paste0("data/hyper_files/U_",names(df_names)[i],"_24_med.bed"),format="bed", ignore.strand = FALSE)
...
median_24_lfc  %>%
  ...
  separate_wider_delim(.,peak, names = c( "chr","start","end"), delim= ".",cols_remove = FALSE) %>%
     ...
     mutate(strand=".") %>% 
     GRanges() %>%
     rtracklayer::export(., con=paste0("data/hypo_files/D_",names(df_names)[i],"_24_med.bed"),format="bed", ignore.strand = FALSE)
}      
...
EARU3h_resized <- resize(EARU_3h_gr, width = 1000,fix='center')
ESRU3h_resized <- resize(ESRU_3h_gr, width = 1000,fix='center')
LRU3h_resized <- resize(LRU_3h_gr, width = 1000,fix='center')
NRU3h_resized <- resize(NRU_3h_gr, width = 1000,fix='center')

EARD3h_resized <- resize(EARD_3h_gr, width = 1000,fix='center')
ESRD3h_resized <- resize(ESRD_3h_gr, width = 1000,fix='center')
LRD3h_resized <- resize(LRD_3h_gr, width = 1000,fix='center')
NRD3h_resized <- resize(NRD_3h_gr, width = 1000,fix='center')

EARU24h_resized <- resize(EARU_24h_gr, width = 1000,fix='center')
ESRU24h_resized <- resize(ESRU_24h_gr, width = 1000,fix='center')
LRU24h_resized <- resize(LRU_24h_gr, width = 1000,fix='center')
NRU24h_resized <- resize(NRU_24h_gr, width = 1000,fix='center')

EARD24h_resized <- resize(EARD_24h_gr, width = 1000,fix='center')
ESRD24h_resized <- resize(ESRD_24h_gr, width = 1000,fix='center')
LRD24h_resized <- resize(LRD_24h_gr, width = 1000,fix='center')
NRD24h_resized <- resize(NRD_24h_gr, width = 1000,fix='center')

BiocManager::install('PWMEnrich')

nEAR_list <- list(EARU3h_resized=EARU3h_resized,EARD3h_resized=EARD3h_resized,EARU24h_resized=EARU24h_resized,EARD24h_resized=EARD24h_resized)

ESR_list <- list(ESRU3h_resized=ESRU3h_resized,ESRD3h_resized=ESRD3h_resized,ESRU24h_resized=ESRU24h_resized,ESRD24h_resized=ESRD24h_resized)

LR_list <- list(LRU3h_resized=LRU3h_resized,LRD3h_resized=LRD3h_resized,LRU24h_resized=LRU24h_resized,LRD24h_resized=LRD24h_resized)

NR_list <- list(NRU3h_resized=NRU3h_resized,NRD3h_resized=NRD3h_resized,NRU24h_resized=NRU24h_resized,NRD24h_resized=NRD24h_resized)

seq_EAR <- lapply(EAR_list, getSeq, x=BSgenome.Hsapiens.UCSC.hg38)

seq_ESR <- lapply(ESR_list, getSeq, x=BSgenome.Hsapiens.UCSC.hg38)


seq_LR <- lapply(LR_list, getSeq, x=BSgenome.Hsapiens.UCSC.hg38)
seq_NR <- lapply(NR_list, getSeq, x=BSgenome.Hsapiens.UCSC.hg38)
```
...
#                         use.names=TRUE)
list_test = lapply(unlist(seq_ESR) , FUN=searchSeq, x=BACH2_pwm, min.score="80%", strand="*")
...
head(writeGFF3(list_test[[1]]))
nolist_test <- searchSeq()
