...
xstreme_EARU %>% 
  left_join(., (sea_EARU %>% dplyr::select(ID:'FP%')), by= c("ALT_ID"="ALT_ID","CONSENSUS"="CONSENSUS", "ID"="ID"))# %>% 
  dplyr::select(CLUSTER, SITES,SIM_MOTIF,ALT_ID, ID,EVALUE.x, TP:'FP%')%>% 
  separate(SIM_MOTIF, into= c("SIM_MOTIF", "NAME"), sep= " ") #%>% 
  arrange(.,EVALUE.x) %>%
 # dplyr::mutate_if(is.numeric, funs(as.character(signif(., 3))))#%>%
...
ggplot(., aes (y= reorder(motif_name,log10Evalue))) +
  geom_col(aes(x=log10Evalue),fill="pink2") +
   geom_point(aes(x=`TP%`), size =5)+
   geom_line(aes(x=`TP%`,y= motif_name, group=log10Evalue))+
  scale_x_continuous(sec.axis = sec_axis(transform= ~./10,name="Percent of peaks with motif"))+
  # geom_text(aes())
  theme_classic()+
  ylab("Enriched TF motif")+
...
xstreme_EARD %>% 
  left_join(., sea_EARD, by= c("ALT_ID"="ALT_ID","CONSENSUS"="CONSENSUS", "ID"="ID")) %>% 
  dplyr::select(CLUSTER, SITES,SIM_MOTIF,ALT_ID, ID,EVALUE.x, TP:'FP%')%>% 
  separate(SIM_MOTIF, into= c("SIM_MOTIF", "NAME"), sep= " ") %>% 
  arrange(.,EVALUE.x) %>%
 # dplyr::mutate_if(is.numeric, funs(as.character(signif(., 3))))#%>%
...
##### ESR hyper
```{r enriched motifs ESR hyper}
xstreme_ESRU %>% 
   left_join(., sea_ESRU, by= c("ALT_ID"="ALT_ID","ID"="ID"))%>% 
  dplyr::select(CLUSTER, SITES,SIM_MOTIF,ALT_ID, ID,EVALUE.x, TP:'FP%')%>% 
  separate(SIM_MOTIF, into= c("SIM_MOTIF", "NAME"), sep= " ") %>% 
  arrange(.,EVALUE.x) %>%
  mutate(log10Evalue= log(EVALUE.x, base = 10)*(-1)) %>% 
  ...
  mutate(motif_name= gsub("[()]","",NAME), mrc="EAR") %>% 
  mutate(motif_name=if_else(is.na(motif_name),ID,motif_name)) %>% 
  mutate(motif_name=if_else(is.na(motif_name),ID,if_else(motif_name=="ZSCAN4",paste(motif_name, RowGroup, sep="_"),motif_name)))%>%
 # dplyr::filter(!is.na(motif_name)) %>% 
  # group_by(mrc) %>% 
ggplot(., aes (y= reorder(motif_name,log10Evalue))) +
  geom_col(aes(x=log10Evalue),fill="lightgreen") +
   geom_point(aes(x=`TP%`), size =4)+
   # geom_line(aes(x=`TP%`,y= motif_name, group=log10Evalue))+
  # geom_text(aes())
  scale_x_continuous(sec.axis = dup_axis(name="Percent of peaks with motif"))+
  theme_classic()+
...
##### ESR hypo
```{r enriched motifs ESR hypo}
xstreme_ESRD %>% 
   left_join(., sea_ESRD, by= c("ALT_ID"="ALT_ID", "ID"="ID"))%>% 
  dplyr::select(CLUSTER, SITES,SIM_MOTIF,ALT_ID, ID,EVALUE.x, TP:'FP%')%>% 
  separate(SIM_MOTIF, into= c("SIM_MOTIF", "NAME"), sep= " ") %>% 
  arrange(.,EVALUE.x) %>%
 # dplyr::mutate_if(is.numeric, funs(as.character(signif(., 3))))#%>%
...
xstreme_LRU %>% 
    left_join(., sea_LRU, by= c("ALT_ID"="ALT_ID", "ID"="ID")) %>% 
  dplyr::select(CLUSTER, SITES,SIM_MOTIF,ALT_ID, ID,EVALUE.x, TP:'FP%')%>% 
  separate(SIM_MOTIF, into= c("SIM_MOTIF", "NAME"), sep= " ") %>% 
  arrange(.,EVALUE.x) %>%
 # dplyr::mutate_if(is.numeric, funs(as.character(signif(., 3))))#%>%
...
xstreme_LRD %>% 
    left_join(., sea_LRD, by= c("ALT_ID"="ALT_ID", "ID"="ID")) %>% 
  dplyr::select(CLUSTER, SITES,SIM_MOTIF,ALT_ID, ID,EVALUE.x, TP:'FP%')%>% 
  separate(SIM_MOTIF, into= c("SIM_MOTIF", "NAME"), sep= " ") %>% 
  arrange(.,EVALUE.x) %>%
 # dplyr::mutate_if(is.numeric, funs(as.character(signif(., 3))))#%>%
