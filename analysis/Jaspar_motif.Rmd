...
xstreme_EARU %>% 
  left_join(., sea_EARU, by= c("ALT_ID"="ALT_ID","CONSENSUS"="CONSENSUS", "ID"="ID")) %>% 
  dplyr::select(CLUSTER, SITES,SIM_MOTIF,ALT_ID, ID,EVALUE.x, TP:'FP%')%>% 
  separate(SIM_MOTIF, into= c("SIM_MOTIF", "NAME"), sep= " ") %>% 
  arrange(.,EVALUE.x) %>%
 # dplyr::mutate_if(is.numeric, funs(as.character(signif(., 3))))#%>%
...
DOX_24 %>%
  ...
  separate_wider_delim(.,peak, names = c( "chr","start","end"), delim= ".",cols_remove = FALSE) %>%
     ...
     mutate(strand=".") %>% 
     GRanges() %>% 
     rtracklayer::export(., con=paste0("data/hyper_files/U_",names(df_names)[i],".bed"),format="bed", ignore.strand = FALSE)
...
DOX_24 %>%
  ...
  separate_wider_delim(.,peak, names = c( "chr","start","end"), delim= ".",cols_remove = FALSE) %>%
     ...
     mutate(strand=".") %>% 
     GRanges() %>% 
     rtracklayer::export(., con=paste0("data/hypo_files/D_",names(df_names)[i],".bed"),format="bed", ignore.strand = FALSE)
}                        
