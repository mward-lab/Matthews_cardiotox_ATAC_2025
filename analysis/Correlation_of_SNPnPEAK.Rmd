...
Wang_SNPS <- read_delim("data/other_papers/wang_bhatia_SNPs.txt", 
    delim = "\t", escape_double = FALSE, 
    trim_ws = TRUE)

Wang_SNPS_df <- Wang_SNPS %>% 
...
```{r}
df_gene_schneider <- data.frame(SYMBOL=c("PRDX6","ANKRD4", "SLC9C2", "TNFSF4", "DARS2", "KLHL20","ZNF460","ZSCAN4", "ROBO2"))
...
```{r Paper data, fig.height=15, fig.width=8}
...
SNPlist <- hm_lfc_df %>% distinct(RSID) %>% ungroup()

SNPlist$HLVgene <- c("DARS2","DARS2","-","-","-","-","-","-","-","-",
                     "-","-","-","-","-","-","-","-","-","-",
                     "-","-","-","-","-","-","-","-","-","-",
                     "-","-","-","PKDREJ","PKDREJ","PKDREJ","PKDREJ","EIF3KP1","EIF3KP1","EIF3KP1",
"-","EIF3KP1","-","ENSG00000279086","-","-","-","-","-","-",
"-","-","-","-","-","-","-","-")

# write.csv(SNPlist,"data/other_papers/schneider_eQTL_genes.csv")
  
```

```{r 50kb region around SNP,fig.height=18, fig.width=8}
...
overlap_df_ggplot %>% 
  dplyr::filter(peakid %in% hm_lfc_df$Peakid)
...
Wang_df <- Wang_SNPS %>% 
 dplyr::rename("RSID"="#Uploaded_variation") %>% 
  ...
  GRanges() %>% as.data.frame 
# another paper analysis

test_ol %>% as.data.frame()
```
