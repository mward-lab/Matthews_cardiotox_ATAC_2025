...
```{r test block, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
...
test <- join_overlap_left(all_peak_gr,cREs_HLV_46F)
test %>% 
  as.data.frame %>% 
  dplyr::filter(is.na(blockCount)) %>% 
  dplyr::select(id) %>% 
  left_join(., median_24_lfc,by=c("id"="peak"))
...
fullOL_medtable %>% 
 mutate(status=factor(status, levels=c("open","closed"))) %>% 
  ...
  geom_boxplot() +
  scale_fill_discrete()+
  ggtitle("EAR peaks and median logFC 24 hours")+
  # facet_wrap(~direction)+
  theme_bw()+
...
fullOL_medtable %>% 
 mutate(status=factor(status, levels=c("open","closed"))) %>% 
  ...
  geom_boxplot() +
  scale_fill_discrete()+
  ggtitle("ESR peaks and median logFC 24 hours")+
  # facet_wrap(~direction)+
  theme_bw()+
  coord_cartesian(xlim=c(-4,4))
fullOL_medtable %>% 
 mutate(status=factor(status, levels=c("open","closed"))) %>% 
  ...
  geom_boxplot() +
  scale_fill_discrete()+
  ggtitle("LR peaks and median logFC 24 hours")+
  # facet_wrap(~direction)+
  theme_bw()+
  coord_cartesian(xlim=c(-4,4))
fullOL_medtable%>% 
 mutate(status=factor(status, levels=c("open","closed"))) %>% 
  ...
  geom_boxplot() +
  scale_fill_discrete()+
  ggtitle("NR peaks and median logFC 24 hours")+
  # facet_wrap(~direction)+
  theme_bw()+
  coord_cartesian(xlim=c(-4,4))
```
### Full Overlap 3 hours
```{r FullOL 3hours}
...
fullOL_medtable_3h %>% 
 mutate(status=factor(status, levels=c("open","closed"))) %>% 
  ...
  geom_boxplot() +
  scale_fill_discrete()+
  ggtitle("EAR peaks and median logFC 3 hours")+
  # facet_wrap(~direction)+
  theme_bw()+
  coord_cartesian(xlim=c(-4,4))
fullOL_medtable_3h %>% 
 mutate(status=factor(status, levels=c("open","closed"))) %>% 
  ...
  geom_boxplot() +
  scale_fill_discrete()+
  ggtitle("ESR peaks and median logFC 3 hours")+
  # facet_wrap(~direction)+
  theme_bw()+
   coord_cartesian(xlim=c(-4,4))
fullOL_medtable_3h %>% 
 mutate(status=factor(status, levels=c("open","closed"))) %>% 
  ...
  geom_boxplot() +
  scale_fill_discrete()+
  ggtitle("LR peaks and median logFC 3 hours")+
  # facet_wrap(~direction)+
  theme_bw()+
   coord_cartesian(xlim=c(-4,4))
fullOL_medtable_3h %>% 
 mutate(status=factor(status, levels=c("open","closed"))) %>% 
  ...
  geom_boxplot() +
  scale_fill_discrete()+
  ggtitle("NR peaks and median logFC 3 hours")+
  # facet_wrap(~direction)+
  theme_bw()+
   coord_cartesian(xlim=c(-4,4))
