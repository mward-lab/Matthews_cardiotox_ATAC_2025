...
#     ggplot2:::manual_scale(
#         'fill', 
#         values = setNames(c( "#8DD3C7", "#FFFFB3", "#BEBADA" ,"#FB8072", "#80B1D3", "#FDB462", "#B3DE69", "#FCCDE5","#D9D9D9"), unique(LTR_family$repFamily)), 
#         ...
#     )
# }
...
TE_mrc_status_list %>% 
  ...
  geom_label_repel(aes(label = n),
                     ...
                     show.legend = FALSE,max.overlaps = 50) +
  theme_void()+
  ggtitle("LINE-L2 breakdown for all peaks",subtitle=paste(all_L2_count," total LINEs"))+
  scale_fill_L2()
...
TE_mrc_status_list %>% 
  ...
  geom_label_repel(aes(label = n),
                     ...
                     show.legend = FALSE,max.overlaps = 50) +
  theme_void()+
  ggtitle("LINE-L2 breakdown for EAR",subtitle=paste(EAR_L2_count$n," total L2s"))+
  scale_fill_L2()
...
TE_mrc_status_list %>% 
  ...
  geom_label_repel(aes(label = n),
                     ...
                     show.legend = FALSE,max.overlaps = 50) +
  theme_void()+
  ggtitle("LINE-L2 breakdown for ESR",subtitle=paste(ESR_L2_count$n," total L2s"))+
  scale_fill_L2()
...
TE_mrc_status_list %>% 
  ...
  geom_label_repel(aes(label = n),
                     ...
                     show.legend = FALSE,max.overlaps = 50) +
  theme_void()+
  ggtitle("LINE-L2 breakdown for LR",subtitle=paste(LR_L2_count$n," total L2s"))+
  scale_fill_L2()
...
TE_mrc_status_list %>% 
  ...
  geom_label_repel(aes(label = n),
                     ...
                     show.legend = FALSE,max.overlaps = 50) +
  theme_void()+
  ggtitle("LINE-L2 breakdown for NR",subtitle=paste(NR_L2_count$n," total L2s"))+
  scale_fill_L2()
...
TE_SINE_count <- TE_mrc_status_list %>% 
  dplyr::filter(TEstatus =="TE_peak"&repClass=="SINE"&per_ol>=.5) %>% 
  count

TE_mrc_status_list %>% 
  dplyr::filter(repClass == "SINE"&per_ol>=.5) %>% 
   mutate(repFamily=factor(repFamily)) %>% 
     group_by(repFamily) %>% 
  count(repFamily) %>% 
   mutate(perc= n/sum(n)) %>% 
...
EAR_SINE_count <- TE_mrc_status_list %>% 
  dplyr::filter(mrc =="EAR"&repClass=="SINE"&per_ol>=.5) %>% 
  count
ESR_SINE_count <- TE_mrc_status_list %>% 
  dplyr::filter(mrc =="ESR"&repClass=="SINE"&per_ol>=.5) %>% 
  count
LR_SINE_count <- TE_mrc_status_list %>% 
  dplyr::filter(mrc =="LR"&repClass=="SINE"&per_ol>=.5) %>% 
  count
NR_SINE_count <- TE_mrc_status_list %>% 
  dplyr::filter(mrc =="NR"&repClass=="SINE"&per_ol>=.5) %>% 
  count
...
Col_fullDF_cug_overlap %>% 
        ...
        geom_density(color="darkblue",fill="lightblue",aes(alpha = 0.5))+
  theme_classic()+
  ggtitle("Distribuiton of CGisland overlap widths",subtitle = " limited x axis")+
  coord_cartesian(xlim= c(0,2500))
...
cpg_island_gr %>% 
  ...
  ggplot(., aes (x = cpg_width))+
        geom_density(aes(alpha = 0.5))+
  theme_classic()+
  ggtitle("Distribuiton of overlap widths of CGislands ",subtitle = " limited x axis")+
  coord_cartesian(xlim= c(0,2500))
