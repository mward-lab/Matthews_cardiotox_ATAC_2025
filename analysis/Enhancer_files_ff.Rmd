...
# peak_list_all_mrc %>% 
#   dplyr::select(chr:Peakid,NCBI_gene:dist_to_NG,mrc9) %>% 
#   GRanges()

# All_enhancers_peaks <- Whole_peaks %>% as.data.frame() %>% 
#   dplyr::filter(blockCount=="CTCF-only,CTCF-bound"|
#                   blockCount =="PLS"|
#                   blockCount =="PLS,CTCF-bound"|
#                   blockCount =="dELS"|
...
# All_enhancers_peaks %>% 
#   left_join(Nine_group_TE,.,by=c("Peakid"="Peakid","mrc"="mrc9")) %>%
#   mutate(TEstatus=if_else(is.na(per_ol),"not_TE_peak","TE_peak")) %>% 
#   dplyr::select(Peakid:repFamily, TEstatus,mrc,NCBI_gene:dist_to_NG,name, blockCount,type) %>% 
#   # dplyr::filter(type=="Proximal enhancer-like"|type=="Distal enhancer-like") %>%
#   distinct() %>% 
#   # dplyr::filter( repClass=="Retroposon") %>% 
#   group_by( mrc,repClass,type) %>% tally %>% 
#   pivot_wider(., id_cols=c(mrc,repClass),names_from=type,values_from = n)
