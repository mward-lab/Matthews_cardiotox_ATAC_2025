...
This is modified to exclude the old cormotif data frames
```{r}

### getting a list of 155557 peaks
...
2) Join the overlap list of regions with the assigned RNA genes and their distance. 

Integration with expressed genes from Matthews, PLOS Genet., 2024
...
The number of overlapping H3K27ac/ATAC peaks and expressed gene pairs within 2kb of and expressed gene TSS is `r Only_2kb_ATAC_K27_RNA %>%  na.omit() %>% distinct(DNR_3_ATAC) %>% tally`
...
```
Looking at correlations of all overlaps, within 2kb of an expressed gene TSS. 
```{r pearsonR-A-Hjust 2kb correlation, fig.height=8}
...
Median_df_2kb %>% 
  ...
  xlab("ATAC peak med LFC")+
   ylab("RNA med LFC")
```
### With and without H3K27ac overlap correlations, 2kb
```{r}
