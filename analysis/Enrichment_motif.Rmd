...
trait 1, 10 associations with trait **cardiotoxicity**
(from 10 rows)
trait 2, 661 associations with trait **cardiac arrhythmia**
(from 1068 rows)

trait 3, 37 associations with trait **response to anthracycline-based chemotherapy**
(from 40 rows)
trait 4, 4068 associations with trait **heart disease**
(from 6139 rows)
trait 5, 398 associations with trait **heart failure**
(from (335 rows))
...
| motif       | gwas_1 (`r length(gwas_1_gr)`)                                        | ARR-gwas_2 (`r length(gwas_2_gr)`)                                      | gwas_3 (`r length(gwas_3_gr)`)                                        | gwas_4 (`r length(gwas_4_gr)`)                                      | Heart Failure (`r length(gwas_5_gr)`)                                        | CAD_gwas (`r length(CAD_gwas_gr)`)                                      | MI_gwas (`r length(MI_gwas_gr)`)
|--------|--------|--------|--------|--------|--------|--------|--------|--------|
| EAR (7321)  | `r length(findOverlaps(EAR_df_gr, gwas_1_gr))` | `r length(findOverlaps(EAR_df_gr, gwas_2_gr_noext))` | `r length(findOverlaps(EAR_df_gr, gwas_3_gr_noext))` | `r length(findOverlaps(EAR_df_gr, gwas_4_gr_noext))` | `r length(findOverlaps(EAR_df_gr, gwas_5_gr_noext))` | `r length(findOverlaps(EAR_df_gr, CAD_gwas_gr))` |`r length(findOverlaps(EAR_df_gr, MI_gwas_gr_noext))`|
| ESR (15665) | `r length(findOverlaps(ESR_df_gr, gwas_1_gr_noext))` | `r length(findOverlaps(ESR_df_gr, gwas_2_gr_noext))` | `r length(findOverlaps(ESR_df_gr, gwas_3_gr_noext))` | `r length(findOverlaps(ESR_df_gr, gwas_4_gr_noext))` | `r length(findOverlaps(ESR_df_gr, gwas_5_gr_noext))` | `r length(findOverlaps(ESR_df_gr, CAD_gwas_gr_noext))` |`r length(findOverlaps(ESR_df_gr, MI_gwas_gr_noext))`|
| LR (41759)  | `r length(findOverlaps(LR_df_gr, gwas_1_gr_noext))`  | `r length(findOverlaps(LR_df_gr, gwas_2_gr_noext))`  | `r length(findOverlaps(LR_df_gr, gwas_3_gr_noext))`  | `r length(findOverlaps(LR_df_gr, gwas_4_gr_noext))`  | `r length(findOverlaps(LR_df_gr, gwas_5_gr_noext))`  | `r length(findOverlaps(LR_df_gr, CAD_gwas_gr_noext))`  |`r length(findOverlaps(LR_df_gr, MI_gwas_gr_noext))`|
| NR (83658)  | `r length(findOverlaps(NR_df_gr, gwas_1_gr_noext))`  | `r length(findOverlaps(NR_df_gr, gwas_2_gr_noext))`  | `r length(findOverlaps(NR_df_gr, gwas_3_gr_noext))`  | `r length(findOverlaps(NR_df_gr, gwas_4_gr_noext))`  | `r length(findOverlaps(NR_df_gr, gwas_5_gr_noext))`  | `r length(findOverlaps(NR_df_gr, CAD_gwas_gr_noext))`  |`r length(findOverlaps(NR_df_gr, MI_gwas_gr_noext))`|

: gwas trait enrichment with no extension

These numbers are different than the original lists from which they derived. This is normally due to running `na.omit()` for rows missing name and location infromation and adding on the `distinct` function to the data frame before GRanges transformation. This reliably removes repeated SNPs. \#

#### chisquare tests

```{r chi numbers}
chitest_gwas2_EAR <- matrix(c(2,7319,48,83610), ncol=2, byrow=TRUE)
...
```{r adding in RNA genes, include=FALSE}
...
```{r CTX SNP by category}
## Heart failure is gwas #5

EAR_CTX_SNP <- as.data.frame(join_overlap_intersect(EAR_df_gr, gwas_1_gr_noext))
ESR_CTX_SNP <- as.data.frame(join_overlap_intersect(ESR_df_gr, gwas_1_gr_noext))
LR_CTX_SNP <- as.data.frame(join_overlap_intersect(LR_df_gr,  gwas_1_gr_noext))
NR_CTX_SNP <- as.data.frame(join_overlap_intersect(NR_df_gr, gwas_1_gr_noext))
...
```{r ACresp SNP by category}
## Heart failure is gwas #5

EAR_ACresp_SNP <- as.data.frame(join_overlap_intersect(EAR_df_gr, gwas_3_gr_noext))
ESR_ACresp_SNP <- as.data.frame(join_overlap_intersect(ESR_df_gr, gwas_3_gr_noext))
LR_ACresp_SNP <- as.data.frame(join_overlap_intersect(LR_df_gr,  gwas_3_gr_noext))
NR_ACresp_SNP <- as.data.frame(join_overlap_intersect(NR_df_gr, gwas_3_gr_noext))
