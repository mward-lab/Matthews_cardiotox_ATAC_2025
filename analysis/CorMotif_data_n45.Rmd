...
### ~made these granges files into a list to annotate and do future analysis
# background_gr <- background %>% as.GRanges()
# 
# mylist <- list(EAR_df_gr,ESR_df_gr,LR_df_gr,NR_df_gr,background_gr)
# peakAnnoList<- lapply(mylist, annotatePeak, tssRegion =c(-2000,2000), TxDb= txdb)
# names(peakAnnoList) <- c("EAR_n45_gr","ESR_n45_gr","LR_n45_gr","NR_n45_gr","background_gr")
# saveRDS(peakAnnoList, "data/peakAnnoList_n45_motif.RDS")


# Eight_group_list <- readRDS("data/TE_info/Eight_group_list.RDS")
# peakAnnoList_Eight<- lapply(Eight_group_list, annotatePeak, tssRegion =c(-2000,2000), TxDb= txdb)
# names(peakAnnoList_Eight) <- c("EAR_hyper","EAR_hypo", "ESR_A", "ESR_B", "ESR_CD", "LR_hyper","LR_hypo","NR")
# 
# saveRDS(peakAnnoList_Eight, "data/peakAnnoList_Eight.RDS")
peakAnnoList_Eight <- readRDS("data/peakAnnoList_Eight.RDS")
plotAnnoBar(peakAnnoList_Eight[c(8,1:7)], main = "Genomic Feature Distribution, CorMotif")+
  ggtitle ("Genomic Feature Distribution, Eight groups")

peakAnnoList_n45_motif <- readRDS("data/peakAnnoList_n45_motif.RDS")
plotAnnoBar(peakAnnoList_n45_motif[c(4,1:3)], main = "Genomic Feature Distribution, CorMotif")+
  ggtitle ("Genomic Feature Distribution, CorMotif")

# Nine_group_TE <- readRDS("data/TE_info/Nine_group_TE_full.RDS")

 # Nine_group_TE %>%
 #   left_join(.,(peakAnnoList_n45_motif$EAR %>%
 #                   as.data.frame %>%
 #                   dplyr::select(id,annotation)), by=c("peakid"="id")) %>%
 #   left_join(.,(peakAnnoList_n45_motif$ESR %>%
 #                   as.data.frame %>%
 #                   dplyr::select(id,annotation)), by=c("peakid"="id")) %>%
 #   left_join(.,(peakAnnoList_n45_motif$LR %>%
 #                   as.data.frame %>%
 #                   dplyr::select(id,annotation)), by=c("peakid"="id")) %>%
 #   left_join(.,(peakAnnoList_n45_motif$NR %>%
 #                   as.data.frame %>%
 #                   dplyr::select(id,annotation)), by=c("peakid"="id")) %>%
 #   dplyr::filter(mrc != "not_mrc") %>%
 #   mutate(annotation= if_else(!is.na(annotation.x),annotation.x,if_else(!is.na(annotation.y),annotation.y,if_else(!is.na(annotation.x.x),annotation.x.x,if_else(!is.na(annotation.y.y),annotation.y.y,"aaaa"))))) %>%
 #   mutate(annotation=factor(annotation)) %>%
 #   dplyr::select(peakid,annotation, mrc) %>%
 #   distinct(annotation)
...
Set <- c("EAR", "ESR","LR", "NR")
gene_num <- c(7321,15667,41767,83741)
...
pie_chartdata %>% 
  ...
  coord_polar("y", pi/2)+
  theme_void()+
  ggtitle("Distribution of peaks for each set")+
...
# vennplot(genes)
plotDistToTSS(peakAnnoList_n45_motif[c(4,1,2,3)])+ ggtitle("Distance of peaks from TSS by accesibility signature")
```
...
# rtracklayer::export.bed(EAR_df_gr, con="data/n45_bedfiles/EAR_df_gr.bed", format="bed", ignore.strand = FALSE)
ESR_df <- as.data.frame(peakAnnoList_n45_motif$ESR_n45_gr)
ESR_promotor <- ESR_df %>% 
  dplyr::filter(annotation=="Promoter (<=1kb)")
ESR_promo_gr <-  GRanges(ESR_promotor)
#  ESR_df_gr <- ESR_df %>% GRanges()
# rtracklayer::export.bed(ESR_df_gr, con="data/n45_bedfiles/ESR_df_gr.bed", format="bed", ignore.strand = FALSE)
LR_df <- as.data.frame(peakAnnoList_n45_motif$LR_n45_gr)
LR_promotor <- LR_df %>% 
  dplyr::filter(annotation=="Promoter (<=1kb)")
LR_promo_gr <-  GRanges(LR_promotor)
# 
#  LR_df_gr <- LR_df %>% GRanges()
# rtracklayer::export.bed(LR_df_gr, con="data/n45_bedfiles/LR_df_gr.bed", format="bed", ignore.strand = FALSE)

NR_df <- as.data.frame(peakAnnoList_n45_motif$NR_n45_gr)
NR_promotor <- NR_df %>% 
  dplyr::filter(annotation=="Promoter (<=1kb)")
  
NR_promo_gr <-  GRanges(NR_promotor)
#  NR_df_gr <- NR_df %>% GRanges()
# rtracklayer::export.bed(NR_df_gr, con="data/n45_bedfiles/NR_df_gr.bed", format="bed", ignore.strand = FALSE)

```



### RNA-seq and highly-expressed Motif response catagories

```{r HEMotif response catagories and DEGRNA}
...
EAR_EAR_peaks <- intersect(EAR_df$geneId,EAR_RNA$ENTREZID)
EAR_promoter_peaks <- intersect(EAR_promotor$geneId,EAR_RNA$ENTREZID)
ESR_ESR_peaks <- intersect(ESR_df$geneId,ESR_RNA$ENTREZID)
ESR_promoter_peaks <- intersect(ESR_promotor$geneId,ESR_RNA$ENTREZID)
LR_LR_peaks <- intersect(LR_df$geneId,LR_RNA$ENTREZID)
LR_promoter_peaks <- intersect(LR_promotor$geneId,LR_RNA$ENTREZID)
NR_NR_peaks <- intersect(NR_df$geneId,NR_RNA$ENTREZID)
NR_promoter_peaks <- intersect(NR_promotor$geneId,NR_RNA$ENTREZID)


toplistall_RNA <- readRDS("data/other_papers/toplistall_RNA.RDS")

###because of DEG analysis in RNA----logFC is backward from counts (this corrects that data)
toplistall_RNA <- toplistall_RNA %>% 
  mutate(logFC = logFC*(-1))

```
  There are `r length (EAR_promoter_peaks)` genes that overlap between the motif response category and DEG in the EAR motif. 
  
  There are `r length (ESR_promoter_peaks)` genes that overlap between the response category and DEG in the ESR motif.  
  
  There are `r length (LR_promoter_peaks)` genes that overlap between the response category and DEG in the LR motif.  
  
  There are `r length( NR_promoter_peaks)` genes that overlap between the motif response category and DEG in the NR motif.
...
Now that I think about it a little more, I wonder if I just need to do all lfc expression, just using from the left data frame, instead of filtering to only genes in common.


```{r all lfc, fig.width=10}
...
EAR_df %>% 
  dplyr::filter(geneId %in% EAR_promotor$geneId) %>% 
  dplyr::select(id,geneId) %>% 
  left_join(.,toplist_n45, by = c("id"="peak" )) %>% 
  dplyr::select(id:logFC) %>% 
  rename(geneId="ENTREZID") %>% 
   na.omit() %>% 
  left_join(., (toplistall_RNA %>% 
  # dplyr:: filter(ENTREZID %in% EAR_promotor$geneId) %>%
  dplyr::select(time:ENTREZID,logFC) %>% mutate(time=factor(time, levels= c("3_hours","24_hours"), labels = c("3 hours","24 hours"))) %>% 
  rename(id="trt")), by= c("ENTREZID", "trt","time")) %>% 
  ggplot(., aes(x=logFC.x,y =logFC.y))+
  geom_point()+
...
ESR_df %>% 
  dplyr::filter(geneId %in%  ESR_promotor$geneId) %>% 
  dplyr::select(id,geneId) %>% 
  left_join(.,toplist_n45, by = c("id"="peak" )) %>% 
  dplyr::select(id:logFC) %>% 
  rename(geneId="ENTREZID") %>% 
...
LR_df %>% 
  dplyr::filter(geneId %in% LR_promotor$geneId) %>% 
  dplyr::select(id,geneId) %>% 
  left_join(.,toplist_n45, by = c("id"="peak" )) %>% 
  dplyr::select(id:logFC) %>% 
  rename(geneId="ENTREZID") %>% 
...
NR_df %>% 
  dplyr::filter(geneId %in% NR_promotor$geneId) %>% 
  dplyr::select(id,geneId) %>% 
  left_join(.,toplist_n45, by = c("id"="peak" )) %>% 
  dplyr::select(id:logFC) %>% 
  rename(geneId="ENTREZID") %>% 
...
```{r testing enhancers, eval=FALSE, include=FALSE}

NR_EH <- subsetByOverlaps((NR_df %>% GRanges(.)),EH_list_gr)

enhancer_list_126UHK <- read_tsv("data/enhancer_list_ENCFF126UHK.bed")

EH_list_gr <- enhancer_list_126UHK %>% 
  GRanges(.)

# # tagMatrixList <- lapply(list(EAR_n45_gr, ESR_n45_gr, LR_n45_gr, NR_n45_gr),getTagMatrix, windows = TSS)
# 
# 
# TTS_matrix <- getTagMatrix(peakAnnoList_3_n45[[4]], 
#                            TxDb = txdb,
#                            upstream = 3000,
#                            downstream = 3000, 
#                            type = "start_site",
...
test <- as.data.frame(NR_EH$annotation)
test %>% 
  dplyr::filter(NR_EH$annotation =="Distal Intergenic")
  count()
...
```
### ATAC and RNA genes in common

```{r comparisonvenns}
library(eulerr)
### Looking at overall expression between neargenes in EAR,ESR,LR, and NR to RNA-genes that are EAR,ESR,LR, NR.
...
# 
# list1 <- EAR_df$geneId
# list2 <- EAR_RNA$ENTREZID

ggVennDiagram::ggVennDiagram(list(unique(as.integer(EAR_df$geneId)),EAR_RNA$ENTREZID),category.names = c("ATAC","RNA"))+
  scale_x_continuous(expand = expansion(mult = .2))+
  scale_fill_gradient(low = "red4", high = "cornflowerblue")+
  labs(title = "EAR motifs in ATAC and RNA")+
  theme(plot.title = element_text(size = rel(1.5), hjust = 0.5))


#specify values to use in venn diagram
fit_EAR <- euler(c('ATAC' = 5526, 'RNA' = 306, 'ATAC&RNA' = 138))
fit_ESR <- euler(c('ATAC' = 9237 , 'RNA'= 336,'ATAC&RNA' =192))
fit_LR <- euler(c('ATAC' =12586 , 'RNA'=2370 ,'ATAC&RNA' =3175))
fit_NR <- euler(c('ATAC' = 17176, 'RNA'= 1406,'ATAC&RNA' =6098))
...
plot(fit_NR,quantities = TRUE,
  ...
  labels = list(font = 6),
  main="NR motifs")                  
# matrixdf <- data.frame(EAR=(c(138)
# 
#  chisq.test(chimatrix2[c(1,4),],correct = FALSE)#$p.value
# chimatrix <- TSS_counts_n45 %>% 
#   dplyr::filter(group != "sanity") %>% 
#   mutate(group=factor(group, levels = c("EAR","ESR","LR","NR"))) %>% 
#   column_to_rownames("group") %>% 
...
ggVennDiagram::ggVennDiagram(list(unique(as.integer(ESR_df$geneId)),ESR_RNA$ENTREZID),category.names = c("ATAC","RNA"))+
  scale_x_continuous(expand = expansion(mult = .2))+
  scale_fill_gradient(low = "coral", high = "cornflowerblue")+
  labs(title = "ESR motifs in ATAC and RNA")+
  theme(plot.title = element_text(size = rel(1.5), hjust = 0.5))

ggVennDiagram::ggVennDiagram(list(unique(as.integer(LR_df$geneId)),LR_RNA$ENTREZID),category.names = c("ATAC","RNA"))+
  scale_x_continuous(expand = expansion(mult = .2))+
  scale_fill_gradient(low = "coral", high = "cornflowerblue")+
  labs(title = "LR motifs in ATAC and RNA")+
  theme(plot.title = element_text(size = rel(1.5), hjust = 0.5))

ggVennDiagram::ggVennDiagram(list(unique(as.integer(NR_df$geneId)),NR_RNA$ENTREZID),category.names = c("ATAC","RNA"))+
  scale_x_continuous(expand = expansion(mult = .2))+
  scale_fill_gradient(low = "coral", high = "cornflowerblue")+
  labs(title = "NR motifs in ATAC and RNA")+
  theme(plot.title = element_text(size = rel(1.5), hjust = 0.5))

Set <- c("EAR", "ESR","LR", "NR")
ypos_resp <- c(138/(138+306)*100,192/(192+336)*100,3175/(2370+3175)*100,6098/(6098+1406)*100)
fills <- c("#F8766D",  "#00BFC4","#7CAE00", "#C77CFF")

pie_chartdata_DAR <- data.frame(Set, ypos_resp, fills)
 
    # pie(gene_num,labels =Set )      

pie_chartdata_DAR <- pie_chartdata_DAR %>% 
   mutate(prop = ypos_resp / sum(pie_chartdata_DAR$ypos_resp) *100) %>%
  mutate(ypos = (prop)+ 0.5*prop )


pie_chartdata_DAR %>% 
  ggplot(.,aes(x="",y=ypos_resp, fill=Set))+
  geom_col(width =1) +
  coord_polar("y", pi/2)+
  theme_void()+
  ggtitle("Proportion of RNA DE genes and Motif response category near-genes")+
  geom_text(aes(label = paste0(Set," (",sprintf("%.1f",ypos_resp),"% )")),
                position = position_stack(vjust =.45)) +
  theme(legend.position="none") +
  theme(plot.title = element_text(size = rel(1.5), hjust = 0.5))
...
##combine of EAR and ESR Motif response categories

response_chrom_gene <-unique(c(unique(ESR_df$geneId),unique(EAR_df$geneId),unique(LR_df$geneId)))
length(unique(LR_df$geneId))
early_dar_df <-
  unique(c(ESR_df$geneId, EAR_df$geneId))
length(intersect((early_dar_df),unique(LR_df$geneId)))
          
length(unique(c(ESR_df$geneId, EAR_df$geneId, LR_df$geneId)))

ggVennDiagram::ggVennDiagram(list(LR_RNA$ENTREZID,
                                  (early_dar_df)),                       
                             category.names = c("RNA\nlate response",
                                                "Chromatin\nearly response"),
                             label="count")+
  scale_x_continuous(expand = expansion(mult = .4))+
  scale_fill_gradient(low = "purple", high = "cornflowerblue")+
  labs(title = "Response genes vs. early chromatin genes")+
  theme(plot.title = element_text(size = rel(1.5), hjust = 0.5))
