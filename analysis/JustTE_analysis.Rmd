...
Class_status_df %>%
  ...
  mutate(Retro_status = if_else(is.na(repClass)&is.na(per_ol),"not_Retro",
                                if_else(repClass=="Retroposon","Retro_peak", "not_Retro"))) %>% 
    dplyr::select(Peakid,Retro_status) %>% 
    mutate(mrc="all_peaks")) %>% 
  group_by(mrc,Retro_status) %>%
  tally %>%
  pivot_wider(., id_cols = mrc, names_from = Retro_status, values_from = n) %>%
  mutate(Per_peak = Retro_peak/(not_Retro+Retro_peak)*100) %>% 
  dplyr::select(mrc, Per_peak) %>%
 
  ggplot(., aes(x=mrc,y=Per_peak)) + 
  geom_col(aes(fill=mrc))+
  scale_fill_manual(values=mrc_palette)+
  theme_bw()
# Class_status_df %>% 
#   group_by(Retro_status) %>% 
...
# Class_status_df %>% distinct(Peakid) %>% tally
# subsetall_df%>% distinct(Peakid) %>% tally
summary <- Nine_group_TE%>% mutate(Retro_status = if_else(is.na(repClass)&is.na(per_ol),"not_Retro",
                                if_else(repClass=="Retroposon","Retro_peak", "not_Retro"))) %>% dplyr::select(Peakid,Retro_status) %>%
  group_by(Peakid) %>% 
    summarise(Retro_status=paste(unique(Retro_status), collapse = ","))
summary %>% 
  # distinct(Peakid,Retro_status) %>%
  group_by(Retro_status) %>% tally
```
...
mrc_lookup_table <- data.frame(mrc=c("EAR_open","ESR_open",  "LR_open","ESR_opcl","EAR_close","ESR_close","LR_close","ESR_clop","NR","all_peaks","h.genome"), 
      peaks = c(3158,5272,28537,99,4296,9349,14072,1061,86331,155560,5974))
...
from https://www.ahajournals.org/doi/10.1161/CIRCRESAHA.120.317254
using 1680 haQTLs from supp VI table

```{r haQTL data import}
# library(readxl)
# haQTL317254 <- read_excel("C:/Users/renee/Downloads/haQTL317254_online_table_vi.xlsx", 
#     sheet = "Sheet1")
# saveRDS(haQTL317254,"data/other_papers/haQTL317254.RDS")
haQTL317254 <- readRDS("data/other_papers/haQTL317254.RDS")

haQTL_gr <- haQTL317254 %>% 
  dplyr::select(`SNP id`,`H3K27ac coordinate`) %>% 
  separate_wider_delim(.,`H3K27ac coordinate`,delim=":", names=c("chr","coordinates"), cols_remove = FALSE) %>% 
  separate_wider_delim(.,coordinates, delim= "-",names=c("start","end")) %>% GRanges()


ha_all <- join_overlap_intersect(TSS_assigned_NG_gr,haQTL_gr)

join_overlap_intersect(TSS_assigned_NG_gr,gwas_snp_gr) %>% as.data.frame() %>% group_by(Peakid,SNPS) %>% tally


ha_EAR_open <- join_overlap_intersect(EAR_open_gr,haQTL_gr)%>% as.data.frame() %>% distinct(Peakid)
ha_EAR_close <- join_overlap_intersect(EAR_close_gr,haQTL_gr)%>% as.data.frame() %>% distinct(Peakid)

ha_LR_open <- join_overlap_intersect(LR_open_gr,haQTL_gr)%>% as.data.frame() %>% distinct(Peakid)
ha_LR_close <- join_overlap_intersect(LR_close_gr,haQTL_gr)%>% as.data.frame() %>% distinct(Peakid)

ha_ESR_open <- join_overlap_intersect(ESR_open_gr,haQTL_gr)%>% as.data.frame() %>% distinct(Peakid)
ha_ESR_close <- join_overlap_intersect(ESR_close_gr,haQTL_gr)%>% as.data.frame() %>% distinct(Peakid)
ha_ESR_opcl <- join_overlap_intersect(ESR_opcl_gr,haQTL_gr)%>% as.data.frame() %>% distinct(Peakid)
ha_ESR_clop <- join_overlap_intersect(ESR_clop_gr,haQTL_gr)%>% as.data.frame() %>% distinct(Peakid)

gwas_snp_list %>% distinct(SNPS)
`````````
