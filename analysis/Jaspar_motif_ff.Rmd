...
# 
# xstreme_ESR_clop200 <- read_delim("~/ATAC_meme_data/200bp/ESR_D_200xstreme/xstreme.tsv",
...
# saveRDS(ESR_D_xstreme,"data/Final_four_data/xstreme/ESR_OC_xstreme.RDS")
...
# sea_ESR_opcl200 <- read_delim("~/ATAC_meme_data/200bp/ESR_C_200xstreme/sea_out/sea.tsv",
#     delim = "\t", escape_double = FALSE,
#     trim_ws = TRUE)
# saveRDS(sea_ESR_opcl200,"data/Final_four_data/xstreme/sea_ESR_opcl_200.RDS")
...
# sea_ESR_opcl_p2 <- read_delim("~/ATAC_meme_data/200bp/ESR_C_200xstreme/sea_disc_out/sea.tsv",
#     delim = "\t", escape_double = FALSE,
#     trim_ws = TRUE)
# saveRDS(sea_ESR_opcl_p2,"data/Final_four_data/xstreme/sea_ESR_opcl_200_p2.RDS")
...
#### EAR 200 bp barplots
```{r enriched motifs EARopen200, fig.height=8}
ER_rat <- 1.5
mrc_type <- "EAR_open"
spec_dataframe_200 %>% 
  dplyr::filter(mrc==mrc_type) %>% 
...
##### EAR_close 200bp
```{r enriched motifs EAR_close200, fig.height=8}
ER_rat <- 1.5
mrc_type <- "EAR_close"
spec_dataframe_200 %>% 
  dplyr::filter(mrc==mrc_type) %>% 
...
```{r enriched motifs ESR_open_200, fig.height=8}
ER_rat <- 1.5
mrc_type <- "ESR_open"
spec_dataframe_200 %>% 
  dplyr::filter(mrc=="ESR_open") %>% 
...
```{r enriched motifs ESR_close_200, fig.height=8}
ER_rat <- 1.5
mrc_type <- "ESR_close"
spec_dataframe_200 %>% 
  dplyr::filter(mrc==mrc_type) %>% 
...
```{r enriched motifs ESR_opcl_200, fig.height=8}
ER_rat <- 1.5
mrc_type <- "ESR_opcl"
spec_dataframe_200 %>% 
  dplyr::filter(mrc==mrc_type) %>% 
...
```{r enriched motifs LR_open_200, fig.height=8}
ER_rat <- 1.5
mrc_type <- "LR_open"
spec_dataframe_200 %>% 
  dplyr::filter(mrc==mrc_type) %>% 
...
```{r enriched motifs LR_close_200, fig.height=8}
ER_rat <- 1.3
mrc_type <- "LR_close"
spec_dataframe_200 %>% 
  dplyr::filter(mrc==mrc_type) %>% 
...
MEIS2_motif_8group %>%
  mutate(id= gsub("_resized","",id)) %>% 
  dplyr::filter(id =="LR_close"|id == "NR"|id=="ESR_close"|id=="EAR_close") %>%
  # dplyr::filter(relScore>0.90) %>%
  # dplyr::filter(id != "ESR_OC") %>% 
  mutate(position=start-200) %>%
  ggplot(.,aes(position, color = id))+
