...
```{r enriched motifs ESR_clop_200, fig.height=8}
ER_rat <- 1.5
mrc_type <- "ESR_clop"
spec_dataframe_200 %>% 
  dplyr::filter(mrc==mrc_type) %>% 
...
nrf1_motif_8group %>%
  ...
  dplyr::filter(id =="EAR_open"|id == "NR"|id =="ESR_close") %>%
...
Atf3_motif_8group %>%
  mutate(id= gsub("_resized","",id)) %>% 
  dplyr::filter(id =="ESR_open"|id == "NR"|id=="EAR_close"|id =="LR_open"|id =="ESR_D") %>%
...
JUNB_motif_8group %>%
  mutate(id= gsub("_resized","",id)) %>% 
   dplyr::filter(id =="LR_open"|id == "NR"|id=="ESR_open"|id =="EAR_close"|id=="ESR_clop") %>%
  # dplyr::filter(relScore>0.90) %>%
  # dplyr::filter(id != "ESR_OC") %>% 
  mutate(position=start-200) %>%
  ggplot(.,aes(position, color = id))+
...
MEIS2_motif_8group %>%
  mutate(id= gsub("_resized","",id)) %>% 
  dplyr::filter(id =="LR_close"|id == "NR"|id=="ESR_close"|id=="EAR_close"|id=="ESR_D") %>%
  dplyr::filter(relScore>0.90) %>%
  # dplyr::filter(id != "ESR_OC") %>% 
  mutate(position=start-200) %>%
  ggplot(.,aes(position, color = id))+
