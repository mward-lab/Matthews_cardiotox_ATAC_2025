...
# saveRDS(FF_bound,"data/Final_four_data/ATAC_seq_counts.RDS")
#  ATAC_seq_counts %>% 
# pivot_wider(., id_cols= c(sample, indv, trt, time),names_from = Total_reads,values_from = read_num) %>% 
#    write_delim(.,file="data/Final_four_data/reads_summary_FF.txt",delim="\t")
```
...
Purity_table <- data_frame("indv"= c("75-1","87-1","77-1","71-1"),
"assign"=c("D","A","B","C"),
"num"= c(1,2,3,6),
...
#### These Ind1 TSS_peaks were created on the "analysis/Peak_calling.RDS" file  example code:
...
hist(filt4_matrix_lcpm)
###  removing chrY (the three rows)


# saveRDS(x4_filtered,"data/Final_four_data/x4_filtered.RDS")
# x4_filtered <- readRDS("data/Final_four_data/x4_filtered.RDS")
...
# new_peaks_gr <- Collapsed_new_peaks %>% GRanges()
heart_overlap <- join_overlap_intersect(Snyder_41peaks,new_peaks_gr)
...
Snyder_41peaks %>% as.data.frame() %>% dplyr::filter(!name %in% heart_overlap_snyder$name)
...
# text(x = 0, y = -0.5, 
#      labels = paste("Overlap in this study:", 66927, "\nOverlap in Snyder study:", 137976), 
#      cex = 0.8, pos = 3)
grid::grid.text(label = paste("Overlap in this study:", 66927, "\nOverlap in Snyder study::",137976),
                x = unit(0.5, "npc"), y = unit(0.1, "npc"), gp = grid::gpar(cex = 0.8))


fit2 <- euler(c("This study" = 1-.43,        
               "Snyder study" = 1-.63,        # Unique to B
               "This study&Snyder study" = min(.43, .63)))

plot(fit2, fills = list(fill = c("skyblue", "lightcoral"), alpha = 0.6),
     labels = FALSE, edges = TRUE, quantities = TRUE,
     main = "Euler diagram between this study and Snyder's study")
grid::grid.text(label = paste("Overlap in this study:", 66927, "\nOverlap in Snyder study::",137976),
                x = unit(0.5, "npc"), y = unit(0.1, "npc"), gp = grid::gpar(cex = 0.8))
...
ggVennDiagram::ggVennDiagram(list(heart_overlap_snyder$name,snyder_peaks$name))
  ggVennDiagram::ggVennDiagram(list(heart_overlap_me$peakid,snyder_peaks$name))
```
...
# saveRDS(efit4,"data/Final_four_data/efit4_filt_norm.RDS")
...
### old style way of making the long Toptable i call toplist_ff
...
# saveRDS(toplist_ff,"data/Final_four_data/toplist_ff_norm.RDS")
toplist_ff <- readRDS("data/Final_four_data/toplist_ff_norm.RDS")
median_3_lfc <- read_csv("data/Final_four_data/median_3_lfc_norm.csv")
median_24_lfc <- read_csv("data/Final_four_data/median_24_lfc_norm.csv")
```
...
write_csv(median_3_lfc, "data/Final_four_data/median_3_lfc_norm.csv")
write_csv(median_24_lfc, "data/Final_four_data/median_24_lfc_norm.csv")
```
...
cormotif_initial_ff <-  readRDS("data/Final_four_data/cormotif_ff_4_run_norm.RDS")
...
saveRDS(motif_prob_ff,"data/Final_four_data/motif_prob_ff_norm.RDS")
...
```{r making clusters}
motif_prob_ff <- readRDS("data/Final_four_data/motif_prob_ff_norm.RDS")
...
ESR_ff <- motif_prob_ff %>%
  ...
  dplyr::select(Peakid) %>% 
  separate(Peakid, into=c("chr","start","end"),remove = FALSE)
 ###to get these peak names, I did a random sample of 2 for each, then pulled the first one of the two.  There were no differences in the two I had picked, but I did the code outside of this box and never put it back in)
...
# saveRDS(peakAnnoList, "data/Final_four_data/peakAnnoList_ff_motif_norm.RDS")

peakAnnoList_motif <- readRDS("data/Final_four_data/peakAnnoList_ff_motif_norm.RDS")
...
```
### Nine group peak characteristics
...
# mylist_new <- list(EAR_open_gr, EAR_close_gr,ESR_open_gr, ESR_close_gr,ESR_opcl_gr,ESR_clop_gr, LR_open_gr, LR_close_gr,NR_gr, background_gr)
# names(mylist_new) <- c("EAR_open", "EAR_close","ESR_open", "ESR_close","ESR_opcl","ESR_clop", "LR_open", "LR_close","NR","background")
...
# saveRDS(peakAnnoList, "data/Final_four_data/peakAnnoList_ff_9motif_norm.RDS")
peakAnnoList_8_motif <- readRDS("data/Final_four_data/peakAnnoList_ff_8motif.RDS")
peakAnnoList_9_motif <- readRDS("data/Final_four_data/peakAnnoList_ff_9motif_norm.RDS")
plotAnnoBar(peakAnnoList_8_motif[c(8,1,3,5,6,2,4,7)])+
  ggtitle ("Genomic Feature Distribution, eight groups")

plotAnnoBar(peakAnnoList_9_motif[c(9,1,3,7,5,2,4,8,6)])+
  ggtitle ("Genomic Feature Distribution, nine groups")

```

#### Distrib. of Peak length by class
```{r Peakwidth distribution by class, fig.width=8}
...
peak_length_df <- new_peaks_gr %>% 
  as.data.frame %>% 
  dplyr::select(peakid,width) 
peak_length_df %>% 
  mutate(mrc = case_when(
    ...
    peakid %in% NR_ff$Peakid ~ "NR",
    peakid %in% ESR_clop$Peakid ~ "ESR_clop",
    TRUE ~ "not_mrc"
  )) %>%  
  dplyr::filter(mrc!="not_mrc") %>% 
  ggplot(., aes(x=mrc, y=width, fill = mrc))+
  geom_boxplot()+
  ggpubr::stat_compare_means(method = "wilcox.test",label= "p.format",step.increase= 0.1,comparisons = list(c("EAR_close","NR"),
                                        c("EAR_open", "NR"),
                                        c("ESR_close","NR"),
                                        c("ESR_open", "NR"),
                                        c("LR_close","NR"),
...
# ENCSR204PZT- Snyder data set GEO:GSE215477 and processed ENCODE4 v1.9.1 GRCh38 processed data-ENCAN548FHQ Accession ENCFF966JZT
# )

Snyder_41peaks <- read.delim("data/other_papers/ENCFF966JZT_bed_Snyder_41peaks.bed",header=TRUE) %>% 
  GRanges()
# overlap_set <- join_overlap_intersect(ATAC_peaks_gr,Snyder_41peaks)
# overlap_set %>% as.data.frame %>% distinct(Peakid)
...
```{r K27 correlation using ATAC-k27 peaks}
...
AC_median_3_lfc <- read_csv("data/Final_four_data/median_3_lfc_norm")
AC_median_24_lfc <- read_csv("data/Final_four_data/median_24_lfc_norm")
...
#### Correlation of LFC
```{r LFC correlation, fig.height=6}
...
```
### LFC Correlation with RNA

```{r addRNA cor, fig.height=5}
...
RNA_ATAC_mat <-   Collapsed_new_peaks %>%
  dplyr::select(Peakid, NCBI_gene,SYMBOL) %>% 
     # dplyr::filter(grepl(",",SYMBOL)|grepl(":",SYMBOL))
  separate_longer_delim(c(NCBI_gene), delim = ",") %>% 
 separate_longer_delim(c(NCBI_gene), delim = ":") %>% 
  separate_longer_delim(c(SYMBOL), delim = ",") %>% 
     separate_longer_delim(c(SYMBOL), delim = ":") %>%
  left_join(.,ATAC_LFC_df,by=c("Peakid"="peak")) %>% 
  left_join(.,RNA_LFC_df,by=c("NCBI_gene"="ENTREZID","SYMBOL"="SYMBOL")) %>% 
  distinct(SYMBOL,NCBI_gene,.keep_all = TRUE) %>% 
  unite(name,Peakid:SYMBOL, sep="_") %>% 
  column_to_rownames("name") %>% 
  as.matrix() 
test_cor_mat <- cor(RNA_ATAC_mat,method = "pearson", use = "pairwise.complete.obs")
  

ComplexHeatmap::Heatmap(test_cor_mat)

```
##### LFC with H3k27AC and RNA AND ATAC LFC

```{r LFC H3K27ac, fig.height=6}
...
bind_frame_ATACK27 <- overlap_df_ggplot %>% 
...
K27_LFC_df <- hr3_K27 %>% 
  ...
  left_join(.,bind_frame_ATACK27, by=c("Geneid"="Geneid")) 
...
RNA_ATAC_K27_mat <-
 bind_frame_ATACK27 %>% 
  ...
  left_join(., K27_LFC_df, by=c("peakid"="peakid", "Geneid"="Geneid")) %>% 
  separate_longer_delim(c(NCBI_gene), delim = ",") %>% 
  separate_longer_delim(c(SYMBOL), delim = ",") %>% 
  left_join(.,ATAC_LFC_df,by=c("peakid"="peak")) %>% 
  left_join(.,RNA_LFC_df,by=c("NCBI_gene"="ENTREZID","SYMBOL"="SYMBOL"))%>% 
  distinct(SYMBOL,NCBI_gene,Geneid,.keep_all = TRUE) %>% 
  unite(name,peakid:SYMBOL, sep="_") %>% 
  column_to_rownames("name") %>% 
  as.matrix() 

All_cor_mat <- cor(RNA_ATAC_K27_mat,method = "pearson", use = "pairwise.complete.obs")
ComplexHeatmap::Heatmap(All_cor_mat)

```


#### Correlation of H3K27ac and ATAC-seq peaks
added 10/14/2024
```{r H3K27ac and ATAC-seq peaks}
joined_LFC_df %>% 
   dplyr::filter(dist_to_NG>-2000 & dist_to_NG<2000) %>% 
  ggplot(., aes(x=med_3h_lfc,y=AC_3h_lfc))+
   geom_point()+
   sm_statCorr(corr_method = 'pearson')+
...
joined_LFC_df %>% 
  ...
  ggplot(., aes(x=med_3h_lfc,y=AC_3h_lfc))+
   geom_point()+
   sm_statCorr(corr_method = 'pearson')+
  ggtitle("Correlation of shared 20kb H3K27ac peaks and ATAC_peaks at 3 hours")+
  ylab("H3K27ac peak LFC")+
   xlab("ATAC-seq log FC")

joined_LFC_df %>% 
   dplyr::filter(dist_to_NG>-2000 & dist_to_NG<2000) %>% 
  ggplot(., aes(x=med_24h_lfc,y=AC_24h_lfc))+
   geom_point()+
   sm_statCorr(corr_method = 'pearson')+
...
joined_LFC_df %>% 
  ...
  ggplot(., aes(x=med_24h_lfc,y=AC_24h_lfc))+
   geom_point()+
   sm_statCorr(corr_method = 'pearson')+
  ggtitle("Correlation of shared 20kb H3K27ac peaks and ATAC_peaks at 24 hours")+
  ylab("H3K27ac peak LFC")+
   xlab("ATAC-seq log FC")
...
ggplot(., aes(x=med_24h_lfc,y=RNA_24h_lfc))+
   geom_point(col="cornflowerblue")+
   sm_statCorr(corr_method = 'pearson')+
   ggtitle("Correlation of 20kb ATAC peaks and RNA-NGat 24 hours")+
  xlab("ATAC peak LFC")+
   ylab("RNA LFC")
...
ggplot(., aes(x=AC_24h_lfc,y=RNA_24h_lfc))+
   geom_point(col="cornflowerblue")+
   sm_statCorr(corr_method = 'pearson')+
   ggtitle("Correlation of all H3K27ac peaks (20kb) and RNA-NG at 24 hours")+
  xlab("H3K27ac peak LFC")+
   ylab("RNA LFC")
```
