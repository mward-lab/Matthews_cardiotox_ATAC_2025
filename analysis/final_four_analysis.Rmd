...
dge<-  dge[!grepl("^chrY", rownames(dge)),,keep.lib.sizes=FALSE ]
...
# cor_matrix <- matrix(NA, nrow = ncol(RNA_LFC_df), ncol = ncol(ATAC_LFC_df))
# for(i in 1:ncol(RNA_LFC_df)) {
#   for(j in 1:ncol(ATAC_LFC_df)) {
#     cor_matrix[i, j] <- cor(RNA_LFC_df[[i]], ATAC_LFC_df[[j]], method = "pearson")
#   }
# }


# Cor_matrix <- cor(Cor_RNA_ATAC_mat, method="pearson")
...
# saveRDS(cormotif_initial_ffl,"data/Final_four_data/cormotif_ff_4_run_noY.RDS")
...
# mylist_new <- list(EAR_open_gr, EAR_close_gr,ESR_open_gr, ESR_close_gr,ESR_C_gr,ESR_D_gr, LR_open_gr, LR_close_gr,NR_gr, background_gr)
